...
/*   Updated: 2025/01/28 21:56:00 by fluzi            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// Funzione per trovare il percorso dell'eseguibile
...
// Funzione per impostare la redirezione dell'input/output
void set_directory(t_comand *cmd, int *fd) {
    ...
    if (cmd->in_file) {
        ...
        if (in_fd >= 0) close(in_fd);  // Chiudi in_fd dopo la duplicazione
    }
    ...
    if (cmd->out_file) {
        ...
        if (out_fd >= 0) close(out_fd);  // Chiudi out_fd dopo la duplicazione
    }
}
...
void exe_func(t_comand *cmd, int *fd) {
    ...
    set_directory(cmd, fd);  // Imposta la redirezione dell'input/output
    ...
    free(path);  // Libera la memoria (non raggiungibile se execve ha successo)
}
