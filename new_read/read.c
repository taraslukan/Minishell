...
/*   Updated: 2025/02/03 15:31:24 by fluzi            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void find_heredoc_type(t_read *r_tools)
{
			...
			|| r_tools->delimiter[length - 1] == '`'))
	{
		...
		char *new_delimiter = ft_strndup(r_tools->delimiter + 1, length - 2);
		if (!new_delimiter)
			return;
...
void    std_here_doc(t_read *r_tools)
{
	...
	while(1)
	{
		...
		if (strcmp(heredoc_line, r_tools->delimiter) == 0)
			break;
		heredoc_line = expand_variables(heredoc_line, r_tools->global_var_enable);
...
void set_struct(t_read *r_tools)
{
	...
	if (!r_tools->std || !r_tools->line || !r_tools->in_file || !r_tools->delimiter)
	{
